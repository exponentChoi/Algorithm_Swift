...
func 카드문자열() {
    ...
    for _ in 0..<n {
        ...
        let chars = readLine()!.split(separator: " ")
        
        for c in chars {
            if array.isEmpty { // 비어있는 경우
                array.append(c)
            } else if array.first! >= c { // 맨 앞자리가 현재 string보다 사전에서 뒤에 있는 경우
                array.insert(c, at: 0)
            } else {
                array.append(c)
            }

...
### 이 문제의 풀이 방법은 아래와 같다.
 ...
 최장 수열을 구한 뒤 전체 수에서 최장 수열을 빼야한다.
 [3, 7, 5, 2, 6, 1, 4] 에서 최장 수열을 찾아보면 [3, 5, 6]이다.
 이것을 기준으로 잡고 나머지를 정렬해주면 최소값이 나온다.
 
 LIS를 알고있다고 해도 어떻게 접근해야 할지 막막하다...
 
 출처 - https://mygumi.tistory.com/142
 */
import Foundation
...
func 줄세우기() {
    let n = Int(readLine()!)!
//    let kids = (0..<n).compactMap { _ in Int(readLine()!) }
    let kids = [3, 7, 5, 2, 6, 1, 4]

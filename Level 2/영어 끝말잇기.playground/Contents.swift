...
func solution(_ n:Int, _ words:[String]) -> [Int] {
    var count = 1 // 몇번 돌았는지 저장하는 변수
    var turn = 1 // 누구 차례인지 명시하는 변수
    var _words = words
    var nowWord = _words.removeFirst() // 첫번째 단어 저장
    var usedWords:[String] = [nowWord] // 사용된 단어에 첫번째 단어 저장
    
    for str in _words {
        // 한바퀴 돈 경우 count를 하나 올려준다.
        if turn == n {
            count += 1
        }
        ...
        // 사용된 단어가 있거나, 끝말잇기가 성립되지 않는경우 break
        if usedWords.contains(str) ||  String(nowWord.last!) != String(str.first!) {
            break
        } else {
            
            usedWords.append(str) // 사용된 단어 변수에 현재 단어 넣기
            nowWord = _words.removeFirst() // words 첫번째 항목을 지우고, 지운 string을 현재 단어로 기록.
        }
    }
    
    // 단어 목록이 비어있는 경우 탈락자가 없는 것이므로 [0, 0] 반환
    if _words.isEmpty {
        return [0, 0]
    }

...
/// 문자 자리수만큼 조합하기.
func combination(str: String, length: Int) -> [String] {
    if str.count < length { return [] } // string이 length보다 짧은 경우 빈값 반환
    if length == 1 { return str.map { String($0) } } // 1자리인 경우 아래 로직을 태우지 않고 바로 반환한다.(시간절약)
    
    guard let first = str.first?.description else { return [] }
    let newString = String(str.dropFirst())
    
    return combination(str: newString, length: length)
        + combination(str: newString, length: length - 1).map { first + $0 }
}

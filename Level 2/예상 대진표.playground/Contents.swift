...
func solution(_ n:Int, _ a:Int, _ b:Int) -> Int {
    var answer = 0
    var one: Double = Double(a)
    var two: Double = Double(b)
    
    while one != two { // 서로 속한 그룹이 같으면 count 종료
        one = ceil(Double(one) / 2) // 올림 함수를 사용해서 해당 숫자가 속한 그룹 번호를 구한다.
        two = ceil(Double(two) / 2)
        answer += 1
    }

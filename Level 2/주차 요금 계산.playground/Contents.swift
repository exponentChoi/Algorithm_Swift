/**
 난이도: Level 2 - 2022 KAKAO BLIND RECRUITMENT
...
struct Fee {
    ...
    init(_ fees: [Int]) {
        guard fees.count == 4 else { return }
        self.basicMin = fees[0]
        self.basicFee = fees[1]
        self.unitMin = fees[2]
        self.unitFee = fees[3]
    }
}
...
struct Car: Hashable {
    ...
    init(record: String) {
        ...
        let currentTime = components[0].time
        self.number = components[1]
        self.lastTime = currentTime
        self.state = Parking(rawValue: components[2]) ?? .out
    }
    
    /// 상태 업데이트
    mutating func update(record: String) {
        let components = record.components(separatedBy: .whitespaces)
        let newState = Parking(rawValue: components[2]) ?? .out
        let currentTime = components[0].time
    ...
    /// 최종 정산금액 반환
    func finalSettlement(fee: Fee) -> Int {
        var finalTime = totalTime
        ...
        return fee.basicFee + calculate(time: finalTime, fee: fee)
    }
    
    // 시간과 요금표로 금액 반환
    private func calculate(time: Int, fee: Fee) -> Int {
        let remainsTime = time - fee.basicMin
        if remainsTime > 0 {
            var units = remainsTime / fee.unitMin
            units += remainsTime % fee.unitMin > 0 ? 1 : 0
            return units * fee.unitFee
        } else {
            return 0
        }
    }
